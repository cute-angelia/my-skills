...
| `WECHAT_HTTP_PROXY` | Optional fixed-egress HTTP/HTTPS proxy, e.g. `http://host-bwg-new.aaqq.in:38152` or `http://user:pass@host:port` |

If not found, prompt the user to provide them and save to `~/.env`.

## Proxy Support

When the user's local IP changes frequently, use a fixed-egress proxy for ALL outbound requests involved in publishing.

Priority order:
1. `WECHAT_HTTP_PROXY` from `~/.env`
2. Existing shell env `HTTPS_PROXY`
3. Existing shell env `HTTP_PROXY`

Apply the proxy consistently to:
- `cgi-bin/token`
- `media/uploadimg`
- `material/add_material`
- `draft/add`
- `https://bm.md/api/markdown/render`
- Any remote-image download step

Recommended shell pattern:

```bash
export WECHAT_HTTP_PROXY="http://127.0.0.1:7890"
export HTTPS_PROXY="$WECHAT_HTTP_PROXY"
export HTTP_PROXY="$WECHAT_HTTP_PROXY"
curl --proxy "$WECHAT_HTTP_PROXY" ...
```

If the proxy is unavailable, fail fast and report that the publishing flow could not complete through the configured proxy.

## Workflow

### Step 0: Resolve Proxy

Before any network call, resolve the proxy from the priority order above.

- If a proxy is configured, use it for every `curl`/HTTP request in this run.
- If no proxy is configured, proceed normally.
- If WeChat returns `40164`, remember the whitelist must contain the PROXY egress IP, not the local machine's current IP.
...
# curl --proxy "$WECHAT_HTTP_PROXY" "https://api.weixin.qq.com/cgi-bin/token?{params}"
```

If error `40164` (IP not in whitelist), instruct user to add the current egress IP at: mp.weixin.qq.com → 设置与开发 → 基本配置 → IP白名单. When using a proxy, this means the proxy server's public IP.
...
# curl --proxy "$WECHAT_HTTP_PROXY" -F media=@image_file ...
```
...
# curl --proxy "$WECHAT_HTTP_PROXY" -F media=@cover_image ...
```

For remote images (e.g. `https://i.ibb.co/...`), download first, then upload to WeChat CDN.

If a proxy is configured, use it for the remote-image download too, so the whole workflow uses the same stable egress.
...
# curl --proxy "$WECHAT_HTTP_PROXY" -H "Content-Type: application/json" -d @draft.json ...
```
...
| `40164` IP whitelist error | Add current egress IP at mp.weixin.qq.com → 基本配置 → IP白名单; with proxy, whitelist the proxy IP |
...
| Proxy set but some requests still fail | Ensure EVERY network request in the run uses the same proxy, including bm.md and remote-image download |
